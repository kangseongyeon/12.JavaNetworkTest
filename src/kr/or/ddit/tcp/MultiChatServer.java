...
public class MultiChatServer {
	// 대화명 및 클라이언트의 Socket 객체를 저장하기 위한 Map 객체변수 선언하기
	...
	class ServerReceiver extends Thread {
		...
		private ServerReceiver(Socket socket) {
			this.socket = socket;
		...
		public void run() {
			try {
				...
				// finally 영역이 실행된다는 것은 클라이언트의 접속이 종료되었다는 의미
				sendMessage('#' + name + "님이 퇴장했습니다.");
				
				// Map에서 해당 사용자 정보 제거하기
				clients.remove(name);
				
				System.out.println("[" + socket.getInetAddress() + " : " + socket.getPort() + "] 에서 접속 종료 하였습니다.");
